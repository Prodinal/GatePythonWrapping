...
from LocalSpacyHu.HunTag3.feature import Feature
from LocalSpacyHu.HunTag3.trainer import Trainer
from LocalSpacyHu.HunTag3.tagger import Tagger
from LocalSpacyHu.HunTag3.transmodel import TransModel
...
def mainTag(featureSet, options, results=None):
    transModel = None
    ...
    if 'inFeatFile' in options and options['inFeatFile']:
        ...
        for sen, comment in tagger.tagCorp(options['inputStream']):
            if results:
                results.tagged_sentences.append((sen, comment))
            else:
                writeSentence(sen, options['outputStream'], comment)
